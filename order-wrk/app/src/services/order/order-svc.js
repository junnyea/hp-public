...
async function createPurchaseOrder({CUST_ID, PROD_ID, QUANTITY}){
  ...
  try{
    ...
    if (result && result.rows.length == 1) {
      ...
      if (orderProdResult && orderProdResult.rows.length == 1) {
        ...
        throw `UNABLE TO CREATE HP_ORDER_PRODUCT PROD_ID(${PROD_ID}),ORDER_ID(${orderId})`;
      }
      
    }else{
      throw `FAILED TO QUERY, NO HP_PRODUCT PROD_ID(${PROD_ID})`;
    }
...
async function createPurchaseOrder({MSG_ID, CUST_ID, PROD_ID, QUANTITY}){
  ...
  try{
    ...
    if (result && result.rows.length == 1) {
      const prodInfo = result.rows[0];
      const ACCEPTED_STATUS = 'ACCEPTED';
      const TOTAL_AMT = prodInfo.price * QUANTITY;
